...
---
An _img_ is a data structure that contains the image array shown on screen, the list of stars detected and measured, some info about the image, like its overall Median and MAD, as well as the limit used during detection (by default 5σ). The SNR is based on the RMS sum of background noise levels (from image MAD), and the Poisson self-noise of the starlight flux. Bright stars are self-limiting, with their SNR growing as Sqrt(Flux). Faint stars are limited by the sky background.
