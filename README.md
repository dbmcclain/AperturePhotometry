...
But now... I relent. I see the virtue in using a Gaussoan core + BG model. Least-squares solution for star amplitude is good. Does not work well on blown-out bright stars, but nothing else could do any better either. And now we are no longer "Aperture Photometry", but rather a fitted Photometry. I decided to switch over because it was apparent that the brightest stars were leaving too much uncounted. So I expanded the Gaussian to a 15x15 overall size, with σ adapted from estimated core sizes of stars. Noise contribution in the wings is not bad, since it is weighted by very low amplitude Gaussian wings at larger distances from the core.
